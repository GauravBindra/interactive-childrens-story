...
# age-5-to-10 bedtime story.  Length ≈ 150 words.
# End scenes 1 & 2 with **exactly two numbered choices** “1.” and “2.”.
# Scene 3 finishes the tale (no choices).
...
central to the scene and mentioned within the first two sentences.**

• For scenes 1 & 2, end with **exactly two numbered choices** (“1.” and “2.”)  
  that the child can pick from for what happens next.  
• Scene 3 should finish the tale (no choices, just a satisfying ending).
...
def generate_scene(scene_no: int, story_so_far: str, last_choice: str) -> str:
    prompt = SCENE_TEMPLATE.format(
        scene_no=scene_no, story_so_far=story_so_far, last_choice=last_choice
    )
...
def start_story(child_prompt, state):
    """Handle the first button press (“Start Story”)."""
    if not child_prompt.strip():
        return (
    ...
    scene = generate_scene(1, child_prompt.strip(), "N/A")
    state = {"scene_no": 1, "story": scene, "last_choice": None}

    # Extract two options for buttons
    opt1, opt2 = _extract_options(scene)

    return (
        gr.update(value=scene),
        gr.update(value=opt1, visible=True),
        gr.update(value=opt2, visible=True),
        gr.update(visible=False),  # restart button hidden
        state,
    )


def choose(option_text, state):
    """Handle option button presses for scenes 1 & 2."""
    scene_no = state["scene_no"] + 1
    story_so_far = state["story"]
    scene = generate_scene(scene_no, story_so_far, option_text)

    # Augment history for coherence
    story_combined = (
    ...
    if scene_no < 3:
        ...
        return (
            ...
            gr.update(visible=False),
            gr.update(visible=False),
            gr.update(visible=True),  # show “New Story” button
            state,
        )
...
def new_story(state):
    """Reset everything."""
    state = {"scene_no": 0, "story": "", "last_choice": "N/A"}
    return (
        gr.update(value=""),
        gr.update(visible=False),
        gr.update(visible=False),
...
def _extract_options(scene_text):
    """Grab the two numbered options from the bottom of a scene."""
    lines = [ln.strip() for ln in scene_text.splitlines()]
    opts = [ln for ln in lines if ln.startswith(("1.", "2."))]
    # Fallback text in case the model misbehaves
    if len(opts) != 2:
...
with gr.Blocks(title="Interactive Bedtime Stories 🌙") as demo:
    gr.Markdown("## 🌙 Interactive Bedtime Stories\n"
                "Tell me what kind of adventure you’d like, "
                "then pick where the tale goes!")

    story_state = gr.State({"scene_no": 0, "story": "", "last_choice": "N/A"})
    ...
    # Two choice buttons (hidden until needed)
    with gr.Row():
