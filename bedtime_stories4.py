...
– now auto-enriches short ideas via enrich_idea.py
"""

import os
import openai
import gradio as gr
from enrich_idea import generate_enriched_idea   # ← NEW
...
• Scenes 1 & 2 must end with **exactly two numbered choices** (“1.” and “2.”)  
...
def _maybe_enrich(raw: str) -> str:
    """Return enriched idea if raw prompt is short; else return raw."""
    if len(raw.split()) < 10:
        try:
            return generate_enriched_idea(raw)
        except Exception as e:            # fallback on any failure
            print("Enrich failed:", e)    # simple log
            return raw
    return raw
...
def start_story(child_prompt, state):
    ...
    if not raw:
        return (gr.update(value="🌙 Please describe the story you want first!"),
